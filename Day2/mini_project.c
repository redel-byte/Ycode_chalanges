...
int main()
{
    ...
    char titre_de_recher[100], new_titre[100];
    char buffer_titre[100], buffer_auteur[100];
    ...
    while (1)
    {
        ...
        case 5:
            ...
            printf("Supprimer: ");
            fgets(buffer_titre, sizeof(buffer_titre), stdin);
            for (int i = 0; i < count; i++)
            {
                if (strcmp(buffer_titre, Titre[i]) == 0)
                {
                    continue;
                }
                else
                {
                    strcpy(buffer_titre[i], Titre[i]);
                    strcpy(buffer_auteur[i], Auteur[i]);
                    buffer_prix[i] = prix[i];
                    buffer_quantite[i] = quantite[i];
                }
            }
